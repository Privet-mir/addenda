...
#helm install -n educhain${NUM} ../educhain-ord/ --namespace mainorg-dev -f ./helm_values/ord${NUM}.yaml
#sleep 30
#ORD_POD=$(kubectl get pods --namespace mainorg-dev -l "app=orderer,release=educhain1" -o jsonpath="{.items[0].metadata.name}")
...
#helm install -n educhain${NUM} ../educhain-ord/ --namespace mainorg-dev -f ./helm_values/ord${NUM}.yaml
#sleep 30
#ORD_POD=$(kubectl get pods --namespace mainorg-dev -l "app=orderer,release=educhain2" -o jsonpath="{.items[0].metadata.name}")
#kubectl logs -n mainorg-dev $ORD_POD | grep 'Starting orderer'
#
echo -e "\e[34m Fabric Peer \e[0m"
export NUM=1
echo -e "\e[34m Install CouchDB chart \e[0m"
helm install -n mainorg-couchdb${NUM} ../educhain-couchdb/ --namespace mainorg-peer -f ./helm_values/cdb-peer${NUM}.yaml
sleep 60
...
helm install -n peer${NUM} ../educhain-peer --namespace mainorg-peer -f ./helm_values/peer${NUM}.yaml
sleep 30
PEER_POD=$(kubectl get pods --namespace mainorg-peer -l "app=educhain,release=peer1" -o jsonpath="{.items[0].metadata.name}")
kubectl logs -n mainorg-peer $PEER_POD | grep 'Starting peer'

echo -e "\e[34m Create Channel \e[0m"
kubectl exec -n mainorg-peer $PEER_POD -- peer channel create -o educhain1-orderer.mainorg-dev.svc.cluster.local:7050 -c educhain-channel -f /hl_config/channel/educhain-channel.tx
kubectl  exec -n mainorg-peer $PEER_POD -- peer channel fetch config /var/hyperledger/educhain-channel.block -c educhain-channel -o educhain1-orderer.mainorg-dev.svc.cluster.local:7050

echo -e "\e[34m Join Channel \e[0m"
kubectl  exec -n mainorg-peer $PEER_POD -- bash -c 'CORE_PEER_MSPCONFIGPATH=$ADMIN_MSP_PATH peer channel join -b /var/hyperledger/educhain-channel.block'

echo -e "\e[34m Install Sample Chaincode \e[0m"
kubectl exec -n mainorg-peer $PEER_POD -- bash -c 'mkdir educhain'
kubectl  exec -n mainorg-peer $PEER_POD -- bash -c 'cd educhain && wget https://raw.githubusercontent.com/hyperledger/fabric-samples/release-1.4/chaincode/chaincode_example02/node/chaincode_example02.js'
kubectl exec -n mainorg-peer $PEER_POD -- bash -c 'cd educhain && wget https://raw.githubusercontent.com/hyperledger/fabric-samples/release-1.4/chaincode/chaincode_example02/node/package.json'

echo -e "\e[34m Install Chaincode \e[0m"
kubectl exec -n mainorg-peer $PEER_POD -- bash -c 'CORE_PEER_MSPCONFIGPATH=/var/hyperledger/admin_msp/ peer chaincode install -n educhain -l node -v 1.0.0 -p educhain'
echo -e "\e[34m Instantiate Chaincode \e[0m"
kubectl exec -n mainorg-peer $PEER_POD -- bash -c "CORE_PEER_MSPCONFIGPATH=/var/hyperledger/admin_msp/ peer chaincode instantiate -o educhain1-orderer.mainorg-dev.svc.cluster.local:7050 -C educhain-channel -n educhain -l node -v 1.0.0 -c '{\"Args\":[\"init\",\"a\",\"100\",\"b\",\"200\"]}'"
sleep 20
echo -e "\e[34m Invoke Chaincode \e[0m"
kubectl exec -n mainorg-peer $PEER_POD -- bash -c "CORE_PEER_MSPCONFIGPATH=/var/hyperledger/admin_msp/ peer chaincode invoke -o educhain1-orderer.mainorg-dev.svc.cluster.local:7050 -C educhain-channel -n educhain  -c '{\"Args\":[\"invoke\",\"a\",\"b\",\"10\"]}'"
sleep 5
echo -e "\e[34m Query Chaincode \e[0m"
kubectl exec -n mainorg-peer $PEER_POD -- bash -c "CORE_PEER_MSPCONFIGPATH=/var/hyperledger/admin_msp/ peer chaincode query -C educhain-channel -n educhain  -c '{\"Args\":[\"query\",\"a\"]}'"
